...
//POST
export const createPatient = async (patient, medicalHistory, sickness, agreement) => {
    try {
        const response = await postAPICall(process.env.REACT_APP_LINK + 'patients', {
            ...
            is_allergic_to_mentioned_drugs: medicalHistory.isAllergictoMentionedDrugs,
...
export const updatePatient = async (id, patient, sickness) => {
    try {
        const response = await putAPICall(process.env.REACT_APP_LINK + 'patients/' + id, {
            ...
            dental_insurance_effective_date: patient.dental_insurance_effective_date,
...
export const getPatient = async (id) => {
    try {
        const response = await getAPICall(process.env.REACT_APP_LINK + 'patients/' + id);
        return ({data:response});   
    } catch (error) {
        return ({error: error});
    }
...
export const searchPatient = async (name) => {
    try {
        const response = await getAPICall(process.env.REACT_APP_LINK + 'patients?searchText=' + name);
        return ({data:response});   
    } catch (error) {
        return ({error: error});
    }
